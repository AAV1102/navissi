/**
 * JS for Module: capacitaciones
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadcapacitacionesData();
});

function refreshcapacitaciones() {
    loadcapacitacionesData();
}

function loadcapacitacionesData() {
    const tableBody = document.getElementById('table-body-capacitaciones');
    const tableHeader = document.getElementById('table-header-capacitaciones');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/capacitaciones/crud.php?action=list')
...
function filtercapacitaciones() {
    const input = document.getElementById('searchcapacitaciones');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-capacitaciones');
