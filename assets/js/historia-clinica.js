/**
 * JS for Module: historia-clinica
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadhistoriaclinicaData();
});

function refreshhistoria-clinica() {
    loadhistoriaclinicaData();
}

function loadhistoriaclinicaData() {
    const tableBody = document.getElementById('table-body-historia-clinica');
    const tableHeader = document.getElementById('table-header-historia-clinica');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/historia-clinica/crud.php?action=list')
...
function filterhistoria-clinica() {
    const input = document.getElementById('searchhistoria-clinica');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-historia-clinica');
