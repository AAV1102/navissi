...
function loaddocumentosData() {
    const tableBody = document.getElementById('table-body-documentos');
    const tableHeader = document.getElementById('table-header-documentos');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/documentos/crud.php?action=list')
...
function filterdocumentos() {
    const input = document.getElementById('searchdocumentos');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-documentos');
