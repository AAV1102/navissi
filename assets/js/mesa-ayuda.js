...
function refreshmesa-ayuda() {
    loadmesaayudaData();
}

function loadmesaayudaData() {
    const tableBody = document.getElementById('table-body-mesa-ayuda');
    const tableHeader = document.getElementById('table-header-mesa-ayuda');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/mesa-ayuda/crud.php?action=list')
...
function filtermesa-ayuda() {
    const input = document.getElementById('searchmesa-ayuda');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-mesa-ayuda');
