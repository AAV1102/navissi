/**
 * JS for Module: exportador-universal
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadexportadoruniversalData();
});

function refreshexportador-universal() {
    loadexportadoruniversalData();
}

function loadexportadoruniversalData() {
    const tableBody = document.getElementById('table-body-exportador-universal');
    const tableHeader = document.getElementById('table-header-exportador-universal');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/exportador-universal/crud.php?action=list')
...
function filterexportador-universal() {
    const input = document.getElementById('searchexportador-universal');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-exportador-universal');
