...
function loadwhatsappData() {
    const tableBody = document.getElementById('table-body-whatsapp');
    const tableHeader = document.getElementById('table-header-whatsapp');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/whatsapp/crud.php?action=list')
...
function filterwhatsapp() {
    const input = document.getElementById('searchwhatsapp');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-whatsapp');
