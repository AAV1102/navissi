...
function loadsistemasData() {
    const tableBody = document.getElementById('table-body-sistemas');
    const tableHeader = document.getElementById('table-header-sistemas');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/sistemas/crud.php?action=list')
...
function filtersistemas() {
    const input = document.getElementById('searchsistemas');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-sistemas');
