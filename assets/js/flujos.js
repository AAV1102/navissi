...
function refreshflujos() {
    loadflujosData();
}

function loadflujosData() {
    const tableBody = document.getElementById('table-body-flujos');
    const tableHeader = document.getElementById('table-header-flujos');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/flujos/crud.php?action=list')
...
function filterflujos() {
    const input = document.getElementById('searchflujos');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-flujos');
