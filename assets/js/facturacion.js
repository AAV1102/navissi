...
function loadfacturacionData() {
    const tableBody = document.getElementById('table-body-facturacion');
    const tableHeader = document.getElementById('table-header-facturacion');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/facturacion/crud.php?action=list')
...
function filterfacturacion() {
    const input = document.getElementById('searchfacturacion');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-facturacion');
