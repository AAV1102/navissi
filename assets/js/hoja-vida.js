...
function refreshhoja-vida() {
    loadhojavidaData();
}

function loadhojavidaData() {
    const tableBody = document.getElementById('table-body-hoja-vida');
    const tableHeader = document.getElementById('table-header-hoja-vida');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/hoja-vida/crud.php?action=list')
...
function filterhoja-vida() {
    const input = document.getElementById('searchhoja-vida');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-hoja-vida');
