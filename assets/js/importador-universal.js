/**
 * JS for Module: importador-universal
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadimportadoruniversalData();
});

function refreshimportador-universal() {
    loadimportadoruniversalData();
}

function loadimportadoruniversalData() {
    const tableBody = document.getElementById('table-body-importador-universal');
    const tableHeader = document.getElementById('table-header-importador-universal');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/importador-universal/crud.php?action=list')
...
function filterimportador-universal() {
    const input = document.getElementById('searchimportador-universal');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-importador-universal');
