/**
 * JS for Module: dashboard-mejorado
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loaddashboardmejoradoData();
});

function refreshdashboard-mejorado() {
    loaddashboardmejoradoData();
}

function loaddashboardmejoradoData() {
    const tableBody = document.getElementById('table-body-dashboard-mejorado');
    const tableHeader = document.getElementById('table-header-dashboard-mejorado');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/dashboard-mejorado/crud.php?action=list')
...
function filterdashboard-mejorado() {
    const input = document.getElementById('searchdashboard-mejorado');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-dashboard-mejorado');
