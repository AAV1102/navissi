...
function loadconfiguracionData() {
    const tableBody = document.getElementById('table-body-configuracion');
    const tableHeader = document.getElementById('table-header-configuracion');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/configuracion/crud.php?action=list')
...
function filterconfiguracion() {
    const input = document.getElementById('searchconfiguracion');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-configuracion');
