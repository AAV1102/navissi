/**
 * JS for Module: agente-multiplataforma
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadagentemultiplataformaData();
});

function refreshagente-multiplataforma() {
    loadagentemultiplataformaData();
}

function loadagentemultiplataformaData() {
    const tableBody = document.getElementById('table-body-agente-multiplataforma');
    const tableHeader = document.getElementById('table-header-agente-multiplataforma');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/agente-multiplataforma/crud.php?action=list')
...
function filteragente-multiplataforma() {
    const input = document.getElementById('searchagente-multiplataforma');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-agente-multiplataforma');
