...
function refreshoffice365() {
    loadoffice365Data();
}

function loadoffice365Data() {
    const tableBody = document.getElementById('table-body-office365');
    const tableHeader = document.getElementById('table-header-office365');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/office365/crud.php?action=list')
...
function filteroffice365() {
    const input = document.getElementById('searchoffice365');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-office365');
