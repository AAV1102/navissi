...
function refreshhuelleros() {
    loadhuellerosData();
}

function loadhuellerosData() {
    const tableBody = document.getElementById('table-body-huelleros');
    const tableHeader = document.getElementById('table-header-huelleros');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/huelleros/crud.php?action=list')
...
function filterhuelleros() {
    const input = document.getElementById('searchhuelleros');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-huelleros');
