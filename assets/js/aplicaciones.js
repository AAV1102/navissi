...
function refreshaplicaciones() {
    loadaplicacionesData();
}

function loadaplicacionesData() {
    const tableBody = document.getElementById('table-body-aplicaciones');
    const tableHeader = document.getElementById('table-header-aplicaciones');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/aplicaciones/crud.php?action=list')
...
function filteraplicaciones() {
    const input = document.getElementById('searchaplicaciones');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-aplicaciones');
