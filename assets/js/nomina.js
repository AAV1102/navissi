...
function loadnominaData() {
    const tableBody = document.getElementById('table-body-nomina');
    const tableHeader = document.getElementById('table-header-nomina');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/nomina/crud.php?action=list')
...
function filternomina() {
    const input = document.getElementById('searchnomina');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-nomina');
