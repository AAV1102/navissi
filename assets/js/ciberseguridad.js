/**
 * JS for Module: ciberseguridad
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadciberseguridadData();
});

function refreshciberseguridad() {
    loadciberseguridadData();
}

function loadciberseguridadData() {
    const tableBody = document.getElementById('table-body-ciberseguridad');
    const tableHeader = document.getElementById('table-header-ciberseguridad');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/ciberseguridad/crud.php?action=list')
...
function filterciberseguridad() {
    const input = document.getElementById('searchciberseguridad');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-ciberseguridad');
