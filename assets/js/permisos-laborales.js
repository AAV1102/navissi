/**
 * JS for Module: permisos-laborales
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadpermisoslaboralesData();
});

function refreshpermisos-laborales() {
    loadpermisoslaboralesData();
}

function loadpermisoslaboralesData() {
    const tableBody = document.getElementById('table-body-permisos-laborales');
    const tableHeader = document.getElementById('table-header-permisos-laborales');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/permisos-laborales/crud.php?action=list')
...
function filterpermisos-laborales() {
    const input = document.getElementById('searchpermisos-laborales');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-permisos-laborales');
