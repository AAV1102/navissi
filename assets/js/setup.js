...
function loadsetupData() {
    const tableBody = document.getElementById('table-body-setup');
    const tableHeader = document.getElementById('table-header-setup');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/setup/crud.php?action=list')
...
function filtersetup() {
    const input = document.getElementById('searchsetup');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-setup');
