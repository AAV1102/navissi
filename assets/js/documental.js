...
function refreshdocumental() {
    loaddocumentalData();
}

function loaddocumentalData() {
    const tableBody = document.getElementById('table-body-documental');
    const tableHeader = document.getElementById('table-header-documental');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/documental/crud.php?action=list')
...
function filterdocumental() {
    const input = document.getElementById('searchdocumental');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-documental');
