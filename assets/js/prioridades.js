...
function refreshprioridades() {
    loadprioridadesData();
}

function loadprioridadesData() {
    const tableBody = document.getElementById('table-body-prioridades');
    const tableHeader = document.getElementById('table-header-prioridades');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/prioridades/crud.php?action=list')
...
function filterprioridades() {
    const input = document.getElementById('searchprioridades');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-prioridades');
