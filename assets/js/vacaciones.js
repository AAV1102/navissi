...
function refreshvacaciones() {
    loadvacacionesData();
}

function loadvacacionesData() {
    const tableBody = document.getElementById('table-body-vacaciones');
    const tableHeader = document.getElementById('table-header-vacaciones');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/vacaciones/crud.php?action=list')
...
function filtervacaciones() {
    const input = document.getElementById('searchvacaciones');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-vacaciones');
