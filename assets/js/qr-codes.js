...
function refreshqr-codes() {
    loadqrcodesData();
}

function loadqrcodesData() {
    const tableBody = document.getElementById('table-body-qr-codes');
    const tableHeader = document.getElementById('table-header-qr-codes');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/qr-codes/crud.php?action=list')
...
function filterqr-codes() {
    const input = document.getElementById('searchqr-codes');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-qr-codes');
