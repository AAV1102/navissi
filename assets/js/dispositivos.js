...
function refreshdispositivos() {
    loaddispositivosData();
}

function loaddispositivosData() {
    const tableBody = document.getElementById('table-body-dispositivos');
    const tableHeader = document.getElementById('table-header-dispositivos');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/dispositivos/crud.php?action=list')
...
function filterdispositivos() {
    ...
    const table = document.getElementById('table-dispositivos');
