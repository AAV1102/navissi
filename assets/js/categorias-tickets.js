/**
 * JS for Module: categorias-tickets
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadcategoriasticketsData();
});

function refreshcategorias-tickets() {
    loadcategoriasticketsData();
}

function loadcategoriasticketsData() {
    const tableBody = document.getElementById('table-body-categorias-tickets');
    const tableHeader = document.getElementById('table-header-categorias-tickets');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/categorias-tickets/crud.php?action=list')
...
function filtercategorias-tickets() {
    const input = document.getElementById('searchcategorias-tickets');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-categorias-tickets');
