/**
 * JS for Module: autodiscovery
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadautodiscoveryData();
});

function refreshautodiscovery() {
    loadautodiscoveryData();
}

function loadautodiscoveryData() {
    const tableBody = document.getElementById('table-body-autodiscovery');
    const tableHeader = document.getElementById('table-header-autodiscovery');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/autodiscovery/crud.php?action=list')
...
function filterautodiscovery() {
    const input = document.getElementById('searchautodiscovery');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-autodiscovery');
