...
function refreshmedico() {
    loadmedicoData();
}

function loadmedicoData() {
    const tableBody = document.getElementById('table-body-medico');
    const tableHeader = document.getElementById('table-header-medico');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/medico/crud.php?action=list')
...
function filtermedico() {
    const input = document.getElementById('searchmedico');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-medico');
