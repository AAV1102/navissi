...
function loadrolesData() {
    const tableBody = document.getElementById('table-body-roles');
    const tableHeader = document.getElementById('table-header-roles');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/roles/crud.php?action=list')
