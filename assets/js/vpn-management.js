/**
 * JS for Module: vpn-management
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadvpnmanagementData();
});

function refreshvpn-management() {
    loadvpnmanagementData();
}

function loadvpnmanagementData() {
    const tableBody = document.getElementById('table-body-vpn-management');
    const tableHeader = document.getElementById('table-header-vpn-management');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/vpn-management/crud.php?action=list')
...
function filtervpn-management() {
    const input = document.getElementById('searchvpn-management');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-vpn-management');
