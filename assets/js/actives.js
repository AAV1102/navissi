...
function loadactivesData() {
    const tableBody = document.getElementById('table-body-actives');
    const tableHeader = document.getElementById('table-header-actives');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/actives/crud.php?action=list')
...
function filteractives() {
    const input = document.getElementById('searchactives');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-actives');
