/**
 * JS for Module: automation-dashboard
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadautomationdashboardData();
});

function refreshautomation-dashboard() {
    loadautomationdashboardData();
}

function loadautomationdashboardData() {
    const tableBody = document.getElementById('table-body-automation-dashboard');
    const tableHeader = document.getElementById('table-header-automation-dashboard');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/automation-dashboard/crud.php?action=list')
...
function filterautomation-dashboard() {
    const input = document.getElementById('searchautomation-dashboard');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-automation-dashboard');
