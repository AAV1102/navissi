...
function refreshenfermeria() {
    loadenfermeriaData();
}

function loadenfermeriaData() {
    const tableBody = document.getElementById('table-body-enfermeria');
    const tableHeader = document.getElementById('table-header-enfermeria');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/enfermeria/crud.php?action=list')
...
function filterenfermeria() {
    const input = document.getElementById('searchenfermeria');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-enfermeria');
