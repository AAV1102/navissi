...
function loadsoporteData() {
    const tableBody = document.getElementById('table-body-soporte');
    const tableHeader = document.getElementById('table-header-soporte');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/soporte/crud.php?action=list')
...
function filtersoporte() {
    const input = document.getElementById('searchsoporte');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-soporte');
