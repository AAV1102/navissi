/**
 * JS for Module: telecomunicaciones
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadtelecomunicacionesData();
});

function refreshtelecomunicaciones() {
    loadtelecomunicacionesData();
}

function loadtelecomunicacionesData() {
    const tableBody = document.getElementById('table-body-telecomunicaciones');
    const tableHeader = document.getElementById('table-header-telecomunicaciones');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/telecomunicaciones/crud.php?action=list')
...
function filtertelecomunicaciones() {
    const input = document.getElementById('searchtelecomunicaciones');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-telecomunicaciones');
