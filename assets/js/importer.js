...
function loadimporterData() {
    const tableBody = document.getElementById('table-body-importer');
    const tableHeader = document.getElementById('table-header-importer');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/importer/crud.php?action=list')
...
function filterimporter() {
    const input = document.getElementById('searchimporter');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-importer');
