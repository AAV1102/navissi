...
function loadequiposData() {
    const tableBody = document.getElementById('table-body-equipos');
    const tableHeader = document.getElementById('table-header-equipos');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/equipos/crud.php?action=list')
...
function filterequipos() {
    const input = document.getElementById('searchequipos');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-equipos');
