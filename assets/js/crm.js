...
function loadcrmData() {
    const tableBody = document.getElementById('table-body-crm');
    const tableHeader = document.getElementById('table-header-crm');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/crm/crud.php?action=list')
...
function filtercrm() {
    const input = document.getElementById('searchcrm');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-crm');
