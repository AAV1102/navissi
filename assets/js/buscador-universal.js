/**
 * JS for Module: buscador-universal
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadbuscadoruniversalData();
});

function refreshbuscador-universal() {
    loadbuscadoruniversalData();
}

function loadbuscadoruniversalData() {
    const tableBody = document.getElementById('table-body-buscador-universal');
    const tableHeader = document.getElementById('table-header-buscador-universal');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/buscador-universal/crud.php?action=list')
...
function filterbuscador-universal() {
    const input = document.getElementById('searchbuscador-universal');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-buscador-universal');
