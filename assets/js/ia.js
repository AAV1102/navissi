...
function loadiaData() {
    const tableBody = document.getElementById('table-body-ia');
    const tableHeader = document.getElementById('table-header-ia');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/ia/crud.php?action=list')
...
function filteria() {
    const input = document.getElementById('searchia');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-ia');
