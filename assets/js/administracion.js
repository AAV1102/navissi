...
function refreshadministracion() {
    loadadministracionData();
}

function loadadministracionData() {
    const tableBody = document.getElementById('table-body-administracion');
    const tableHeader = document.getElementById('table-header-administracion');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/administracion/crud.php?action=list')
...
function filteradministracion() {
    const input = document.getElementById('searchadministracion');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-administracion');
