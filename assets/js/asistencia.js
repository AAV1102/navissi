...
function refreshasistencia() {
    loadasistenciaData();
}

function loadasistenciaData() {
    const tableBody = document.getElementById('table-body-asistencia');
    const tableHeader = document.getElementById('table-header-asistencia');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/asistencia/crud.php?action=list')
...
function filterasistencia() {
    const input = document.getElementById('searchasistencia');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-asistencia');
