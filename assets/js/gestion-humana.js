/**
 * JS for Module: gestion-humana
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadgestionhumanaData();
});

function refreshgestion-humana() {
    loadgestionhumanaData();
}

function loadgestionhumanaData() {
    const tableBody = document.getElementById('table-body-gestion-humana');
    const tableHeader = document.getElementById('table-header-gestion-humana');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/gestion-humana/crud.php?action=list')
...
function filtergestion-humana() {
    const input = document.getElementById('searchgestion-humana');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-gestion-humana');
