/**
 * JS for Module: n8n-automation
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadn8nautomationData();
});

function refreshn8n-automation() {
    loadn8nautomationData();
}

function loadn8nautomationData() {
    const tableBody = document.getElementById('table-body-n8n-automation');
    const tableHeader = document.getElementById('table-header-n8n-automation');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/n8n-automation/crud.php?action=list')
...
function filtern8n-automation() {
    const input = document.getElementById('searchn8n-automation');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-n8n-automation');
