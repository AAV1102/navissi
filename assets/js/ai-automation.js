/**
 * JS for Module: ai-automation
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadaiautomationData();
});

function refreshai-automation() {
    loadaiautomationData();
}

function loadaiautomationData() {
    const tableBody = document.getElementById('table-body-ai-automation');
    const tableHeader = document.getElementById('table-header-ai-automation');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/ai-automation/crud.php?action=list')
...
function filterai-automation() {
    const input = document.getElementById('searchai-automation');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-ai-automation');
