...
function loadautomationData() {
    const tableBody = document.getElementById('table-body-automation');
    const tableHeader = document.getElementById('table-header-automation');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/automation/crud.php?action=list')
...
function filterautomation() {
    const input = document.getElementById('searchautomation');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-automation');
