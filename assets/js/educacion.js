...
function refresheducacion() {
    loadeducacionData();
}

function loadeducacionData() {
    const tableBody = document.getElementById('table-body-educacion');
    const tableHeader = document.getElementById('table-header-educacion');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/educacion/crud.php?action=list')
...
function filtereducacion() {
    const input = document.getElementById('searcheducacion');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-educacion');
