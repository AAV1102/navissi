...
function refreshdiagnostico() {
    loaddiagnosticoData();
}

function loaddiagnosticoData() {
    const tableBody = document.getElementById('table-body-diagnostico');
    const tableHeader = document.getElementById('table-header-diagnostico');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/diagnostico/crud.php?action=list')
...
function filterdiagnostico() {
    const input = document.getElementById('searchdiagnostico');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-diagnostico');
