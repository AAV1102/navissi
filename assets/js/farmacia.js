...
function refreshfarmacia() {
    loadfarmaciaData();
}

function loadfarmaciaData() {
    const tableBody = document.getElementById('table-body-farmacia');
    const tableHeader = document.getElementById('table-header-farmacia');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/farmacia/crud.php?action=list')
...
function filterfarmacia() {
    const input = document.getElementById('searchfarmacia');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-farmacia');
