...
function refreshmikrotik() {
    loadmikrotikData();
}

function loadmikrotikData() {
    const tableBody = document.getElementById('table-body-mikrotik');
    const tableHeader = document.getElementById('table-header-mikrotik');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/mikrotik/crud.php?action=list')
...
function filtermikrotik() {
    const input = document.getElementById('searchmikrotik');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-mikrotik');
