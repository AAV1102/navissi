...
function refreshlicencias() {
    loadlicenciasData();
}

function loadlicenciasData() {
    const tableBody = document.getElementById('table-body-licencias');
    const tableHeader = document.getElementById('table-header-licencias');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/licencias/crud.php?action=list')
...
function filterlicencias() {
    ...
    const table = document.getElementById('table-licencias');
