...
function refreshcontratos() {
    loadcontratosData();
}

function loadcontratosData() {
    const tableBody = document.getElementById('table-body-contratos');
    const tableHeader = document.getElementById('table-header-contratos');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/contratos/crud.php?action=list')
...
function filtercontratos() {
    const input = document.getElementById('searchcontratos');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-contratos');
