...
function loadsstData() {
    const tableBody = document.getElementById('table-body-sst');
    const tableHeader = document.getElementById('table-header-sst');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/sst/crud.php?action=list')
...
function filtersst() {
    const input = document.getElementById('searchsst');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-sst');
