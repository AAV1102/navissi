...
function loadalertasData() {
    const tableBody = document.getElementById('table-body-alertas');
    const tableHeader = document.getElementById('table-header-alertas');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/alertas/crud.php?action=list')
...
function filteralertas() {
    const input = document.getElementById('searchalertas');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-alertas');
