/**
 * JS for Module: servicio-cliente
 * Auto-generated for Atera integration
 */

document.addEventListener('DOMContentLoaded', function() {
    loadservicioclienteData();
});

function refreshservicio-cliente() {
    loadservicioclienteData();
}

function loadservicioclienteData() {
    const tableBody = document.getElementById('table-body-servicio-cliente');
    const tableHeader = document.getElementById('table-header-servicio-cliente');

    if(!tableBody) return;

    tableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary" role="status"></div></td></tr>';

    fetch('../../api/servicio-cliente/crud.php?action=list')
...
function filterservicio-cliente() {
    const input = document.getElementById('searchservicio-cliente');
    const filter = input.value.toLowerCase();
    const table = document.getElementById('table-servicio-cliente');
