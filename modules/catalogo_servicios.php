    ...
    try {
        ...
        if ($accion === 'guardar_servicio') {
            ...
            $datos = [
                ...
                limpio($_POST['area_tramite'] ?? null),
            ];
            if (!$codigo || !$datos[1] || !$datos[4]) throw new InvalidArgumentException('Código, nombre y área son obligatorios.');
            if ($id) {
                $datos[] = $id;
                $pdo->prepare('UPDATE catalogo_servicios SET codigo=?,nombre=?,descripcion=?,categoria=?,area_responsable=?,nivel_aprobacion=?,requiere_monto=?,monto_escalamiento=?,sla_horas=?,crea_ticket=?,categoria_ticket=?,prioridad_ticket=?,orden=?,area_tramite=?,actualizado_en=CURRENT_TIMESTAMP WHERE id=?')->execute($datos);
            } else {
                $pdo->prepare('INSERT INTO catalogo_servicios(codigo,nombre,descripcion,categoria,area_responsable,nivel_aprobacion,requiere_monto,monto_escalamiento,sla_horas,crea_ticket,categoria_ticket,prioridad_ticket,orden,area_tramite) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)')->execute($datos);
            }
