...
requiere_login('../');
$id = (int) ($_GET['id'] ?? 0);
$stmt = $pdo->prepare("SELECT * FROM movimientos_documentos WHERE id = ?");
$stmt->execute([$id]);
$d = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$d || !$d['ruta']) { http_response_code(404); exit('No encontrado.'); }
$ruta = __DIR__ . '/../data/movimientos_documentos/' . $d['ruta'];
$real = realpath($ruta);
$base = realpath(__DIR__ . '/../data/movimientos_documentos');
